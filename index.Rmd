...
male_df <- tibble::tibble(編號 = seq_along(fps_m), 歌名 = name_m, 歌詞 = lyric_m)
head(male_df)
female_df <- tibble::tibble(編號 = seq_along(fps_f), 歌名 = name_f, 歌詞 = lyric_f)
head(female_df)
...
stop_words <- read_excel("stop_words.xlsx")

tidy_male <- male_df %>%
  unnest_tokens(output = '用詞', input = '歌詞', token = 'regex', pattern = ' ') %>%
  anti_join(stop_words, by = c('用詞' = 'stop_words'))
